...
Mediremos a **longevidade** das séries através do número de episódios produzidos. Ou seja, as séries com maior longevidade são as que possuem mais episódios.  
...
series_by_longevity_and_quality %>%
    ggplot(aes(x = numero_episodios, y = avaliacao)) +
        ...
        geom_point() +
        labs(x = "Número de episódios", y = "Avaliação")
```
...
most_popular_episodes %>%
    ...
    labs(x = "Episódios", y = "Porporção de notas 10") +
    guides(fill = guide_legend(title = "Séries"))
```
...
most_popular_episodes %>%
    ...
    geom_bar(stat = "identity") + coord_flip() +
    labs(x = "Episódios", y = "Porporção de notas 1") +
    guides(fill = guide_legend(title = "Séries"))
```
...
series_with_most_popular_episodes %>%
    ...
    geom_bar(stat = "identity") + coord_flip() +
    labs(x = "Séries", y = "Número de Episódios")
```
...
#Comentários
  ...
  Relatório muito bom e com o raciocínio bem explicado, facilitando muito o entendimento. A resposta da pergunta gerada no final poderia ter sido mais bem explicada, mas não atrapalha muito. Deixei algumas sugestões com relação as amostras aleatórias e as legendas dos gráficos.